...
app.io.route('newPlayer', function(req) {
    ...
    req.socket.playerID = nextPlayerID;
    ...
    req.io.broadcast('newPlayerConnected', {id: playersInfos[playerID]["id"], name: playersInfos[playerID]["name"]});
});
...
var cycle = function() {
    ...
    if(etatJeu == 1) {
        if(gameLaunchTime > 0) {
            if(gameLaunchTime % fps == 0) {
                console.log("Début de la partie dans : "+(gameLaunchTime/fps));
...
var runPlayers = function() {
    ...
    for(var i in colisions) {
        ...
        playersInfos[colisions[i]].alive = false;
    }
...
var demarrerPartie = function() {
    ...
    for(var player in playersInfos) {
        ...
        playersInfos[player]["position"]["direction"] = 1;
