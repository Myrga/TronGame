...
app.controller("TronController", function($http, $scope){
    var ctrl = this;
    ctrl.io = io.connect();
    ctrl.players = {};
...
// Paramètres des annonces
    ...
    ctrl.io.on('init', function(data) {
        ...
        $scope.$apply(function(){
            for(var player in data.players){
                ctrl.players[player] = {name: data.players[player].name, alive: data.players[player].alive};
            }
            ctrl.players[playerInfos.id] = {name: playerInfos.name, alive: playerInfos.alive};
        });
    });
    ...
    ctrl.io.on('newPlayerConnected', function(newPlayer) {
        audioNewPlayer.play();
        $scope.$apply(function(){
            ctrl.players[newPlayer.id] = {name: newPlayer.name, alive: true};
        });
    ...
    ctrl.io.on('disconnect', function(idPlayerDisconnected){
        $scope.$apply(function(){
            delete ctrl.players[idPlayerDisconnected];
        });
    });

    ctrl.io.on('colision', function(colisions) {
